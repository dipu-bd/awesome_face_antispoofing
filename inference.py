...
class DataHandle():
    ...
    def crop_with_ldmk(self, image, landmark):
        ...
        result = cv2.warpAffine(image, retval, (self.image_size, self.image_size),
...
def inference(**kwargs):
    ...
    opt.parse(kwargs)
    # 模型
    opt.load_model_path = getattr(opt, 'load_model_path', pths[0])
    ...
    if opt.use_gpu:
        model.cuda()
    model.train(False)
    fopen = open(os.path.join('result', 'inference.txt'), 'w')
    ...
    for idx, imgdir in tqbar:
        ...
        with torch.no_grad():
            ...
            tqbar.set_description(desc='Inference %s attack_prob=%f with %s' %
                                  (imgdir, attack_prob, opt.model))
