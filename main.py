...
def train(**kwargs):
    # Update configuration based on command line parameters
    opt.parse(kwargs)
    ...
    # step3: Objective function and optimizer
    ...
    # step4: Statistical indicators: loss after smoothing, and confusion matrix
    ...
    for epoch in range(opt.max_epoch):
        ...
        if tpr1 > best_tpr:
            ...
            model_dir = os.path.join('checkpoints', opt.model)
            os.makedirs(model_dir, exist_ok=True)
            model.save(name=os.path.join(model_dir, str(epoch) + '.pth'))
            # print('Epoch: {:d} Val Loss: {:.8f} Acc: {:.4f}'.format(epoch,v_loss,v_accuracy),file=open(os.path.join('result', 'val.txt'),'a'))
            print('Epoch: {:d} Val Loss: {:.8f} Acc: {:.4f} EER: {:.6f} TPR(1.0%): {:.6f} TPR(.5%): {:.6f} AUC: {:.8f}'.format(
                epoch, v_loss, v_accuracy, eer, tprs["TPR(1.%)"], tprs["TPR(.5%)"], auc), file=open(os.path.join('result', 'val.txt'), 'a'))
            print('Epoch: {:d} Val Loss: {:.8f} Acc: {:.4f} EER: {:.6f} TPR(1.0%): {:.6f} TPR(.5%): {:.6f} AUC: {:.8f}'.format(
...
#				test = True,transform = data_transforms['test'])
    ...
    auc = metric[2]
    xy_dic = metric[3]
    pickle.dump(xy_dic, open(os.path.join('result', 'xy.pickle'), 'wb'))
    print('EER: {:.6f} TPR(1.0%): {:.6f} TPR(.5%): {:.6f} AUC: {:.8f}'.format(
        eer, tprs["TPR(1.%)"], tprs["TPR(.5%)"], auc), file=open(os.path.join('result', 'test.txt'), 'a'))
    print('EER: {:.6f} TPR(1.0%): {:.6f} TPR(.5%): {:.6f} AUC: {:.8f}'.format(
